...
<template>
  <Teleport to="body">
    ...
    <div v-if="isOpen" @click="isOpen = false; isDropdownOpen = false" class="fixed inset-0 bg-black/40 backdrop-blur-[2px] z-[70] transition-opacity duration-500"></div>
      ...
      <div class="flex flex-col gap-10">
        ...
        <div class="relative">
          ...
          <div 
            ...
            class="w-full bg-transparent border-b border-gray-300 dark:border-white/20 hover:border-theme-gold dark:hover:border-theme-gold py-2 text-xs font-light cursor-pointer transition-colors text-gray-500 dark:text-white/0 flex justify-between items-center"
          >
          ...
          >
            <div 
              v-if="isDropdownOpen" 
              class="absolute top-full left-0 w-full mt-2 backdrop-blur-2xl bg-white/90 dark:bg-theme-bg/90 border border-gray-200/50 dark:border-white/10 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] z-50 overflow-hidden"
            >
              <div 
                v-for="category in availableCategories" 
                :key="category" 
                @click="emit('update:selectedCategory', category); isDropdownOpen = false"
                class="px-4 py-3 text-sm font-light cursor-pointer transition-colors"
